...
def decode_json(json_floder_path, json_name, classes, output_dir='labels'):
    classes = classes.split(',') if isinstance(classes, str) else classes
    os.makedirs(output_dir, exist_ok=True)
    txt_name = os.path.join(output_dir, json_name[0:-5] + '.txt')
...
if __name__ == "__main__":
    ...
    classes = "reflective_clothes,hat,person"
    for json_name in json_names:
        decode_json(json_floder_path, json_name,classes)
